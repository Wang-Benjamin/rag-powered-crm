...
Copied from prelude-crm/email_service/outreach/bol_intelligence.py — pure computation, no DB deps.
"""
...
def _build_supplier_vulnerability(supplier_ctx: Optional[dict]) -> Optional[dict]:
    ...
    for s in cn_suppliers:
        ...
        if trend and trend < -10:
            declining.append({
                ...
                "shipments12m": s.get("shipments12m", 0),
                "shipments12_24m": s.get("shipments12_24m", 0),
            })
    ...
    result = {
        "primarySupplier": {
            ...
            "shipments12m": primary.get("shipments12m", 0),
            "shipments12_24m": primary.get("shipments12_24m", 0),
        },
    ...
    if primary_trend and primary_trend < -10:
        ...
        result["summary"] = (
            ...
            f"({primary.get('shipments12_24m', '?')}→{primary.get('shipments12m', '?')} shipments). "
            f"Potential supply gap."
        )
...
def build_bol_intelligence(
    ...
    Returns a dict with the 4 intelligence angles + composeSummary.
    """
