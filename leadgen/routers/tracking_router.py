...
from data.connection import lookup_db_name, get_tenant_conn
...
async def track_email_open(t: str, e: Optional[str] = Query(None)):
    ...
    try:
        ...
        if not user_email:
            # Cannot route without user email - just return pixel
            logger.warning("No user email for tracking pixel - skipping DB update")
        else:
            # Look up tenant DB and use async connection
            db_name = await lookup_db_name(user_email)
            async with get_tenant_conn(db_name) as conn:
