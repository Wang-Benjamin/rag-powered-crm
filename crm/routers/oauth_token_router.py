...
async def save_oauth_tokens(
    ...
    Args:
        ...
        tenant: Tenant connection tuple (conn, user) from dependency
    ...
    try:
        ...
        # Create token manager with tenant connection
...
async def refresh_oauth_token(
    ...
    try:
        ...
        if not access_token:
            raise HTTPException(
                status_code=404,
                detail=f"No {provider} tokens found for user. Please re-authenticate."
            )

        logger.info(f"Refreshed {provider} token for {user_email}")
